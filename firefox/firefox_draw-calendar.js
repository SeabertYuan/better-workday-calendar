...
function addFilterButtons() {
  const headerContents = document.getElementsByClassName(
    "css-fgks37-HeaderContents e15c5nta2",
  );
  // title and x button
  const existingDivs = headerContents[0].children;

  const buttonDiv = document.createElement("div");
  buttonDiv.className = "term-filter-buttons";

  const buttons = ["Term 1", "Term 2"];
  buttons.forEach((text, index = 0) => {
